...
if ($congresses)
{
    foreach ($congresses as $congress)
    {
        ...
        {
            array_push($futureCongresses, $congress);
        }
    }
    if (count($futureCongresses) > 0)
    {
        echo packageDataForUnity($format, $futureCongresses, "congresses");
    }
    else
    {
        echo packageTypeErrorForUnity($format, "There are no congresses to display.", 1);
    }
}
else
{
